			...
			<div class="col-md-12 text-center tabs">
				...
				<nav class="nav-left ">
					<div class="nav nav-tabs " id="nav-tab" role="tablist">
						...
						<a class="tab-item nav-item nav-link" id="pop4-tab" data-toggle="tab" href="#pop4" role="tab" aria-controls="pop4" aria-selected="false">API</a>
					</div>
				</nav>
						...
						<div class="row">
							...
							<div class="mb-4 mt-2 filter">
								<form method="get" class="row">
									...
									<a href="{{ route('node.view', $thisNode->id) }}" class="btn btn-success btn-sm btn-filter ml-3" style="color: #fff">Live monitoring</a>
									@endif
								</form>
								@if(!isset($_GET['start']) || !isset($_GET['end']))
							...
							<div class="col-md-6 mt-3 chart-wrapper">
								...
								<p style="margin-left: 15px" class="float-right view-kode text-success"><a href="{{ route('annotation.index', $sensor->id) }}">Kelola event</a></p>
								...
								<div class="sensor-sum">
									...
									<div class="row">
										<div class="col-md-12">
											<a target="__blank" href="{{ route('ral.select', $sensor->id) }}" class="btn btn-sm btn-success">Hitung RAL</a>
										</div>
									</div>
...
<div class="modal fade" id="add-annotation">
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<form method="post" action="{{ route('annotation.post') }}">
				@csrf
				
				<div class="modal-body">
					<p>Event pada <span id="ann-label-label"></span></p>
					<div class="setting-item">
						<input placeholder="Event" type="text" class="form-control" name="value" required>
					</div>
					<input id="ann-sensor" type="hidden" name="sensor_id" required>
					<input id="ann-sensor-val" type="hidden" name="sensor_val" required>
					<input id="ann-label" type="hidden" name="label" required>
				</div>
 ...
 */
 (function ($) { 
 	...
 	$('#show').on('change', function() {
 		var val = $(this).val();
 		if(val == "0") {
 			window.location = "/view/{{ $thisNode->id }}";
 		}else{
 			window.location = "/view/{{ $thisNode->id }}?limit=" + val;
 		}
...
<script>
	...
	var config_{{ $sensor->id }} = {
		...
		options: {
			...
			annotation: {
				...
				@endforeach
				@endif
				@if($sensor->limit_values)
				@foreach(json_decode($sensor->limit_values) as $limit)
				// {
				// 	type: 'line',
				// 	mode: 'horizontal',
				// 	scaleID: 'y-axis-0',
	...
	var canvas_{{ $sensor->id }} = document.getElementById("line-chart-{{ $sensor->id }}");
	var context_{{ $sensor->id }} = canvas_{{ $sensor->id }}.getContext("2d");
	var chart_{{ $sensor->id }} = new Chart(context_{{ $sensor->id }}, config_{{ $sensor->id }});

	canvas_{{ $sensor->id }}.onclick = function (evt) {
	    var points = chart_{{ $sensor->id }}.getElementAtEvent(evt)[0];
	    var val = chart_{{ $sensor->id }}.data.labels[points._index];
	    var sensor_id = {{ $sensor->id }};
	    var sensor_val = chart_{{ $sensor->id }}.data.datasets[0].data[points._index];
	    $('#ann-label-label').html(val);
	    $('#ann-label').val(val);
	    $('#ann-sensor').val(sensor_id);
	    $('#ann-sensor-val').val(sensor_val);
	    $('#add-annotation').modal('show');
	};
		...
		$.ajax({
			...
			success:function(data){
				...
				if(oldNotifCount != newNotifCount && newNotifCount != 0) {
					toastr["info"]("Anda mendapatkan notifikasi baru", "Notifikasi");
				}
